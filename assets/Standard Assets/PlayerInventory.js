...
//array of Item used for display in inventory
...
//used to set starting stats
public var startATK : int;
public var startAGI : int;
public var startINT : int;
public var startLCK : int;
//used to add stats gained from level up
public var levelUpATK: int;
public var levelUpAGI: int;
public var levelUpINT: int;
public var levelUpLCK: int;
...
//toggle open and closed of inventory
function Update () : void {
	if(Input.GetKeyDown(KeyCode.M)){
		b_openMenu = !b_openMenu;
	}
//  change stats on level up	
//	if(currentNEXT <= 0){
//		currentNEXT = 100;
//		fullHP += 50;
//		fullMP += 50;
...
//these methods change the booleans for what item is in inventory
//will replace current item with new item
...
//first page, status of inventory GUI
...
//ensure player stats don't exceed the max
...
//responsible for formatting and drawing the item tab of inventory
private function ItemWindow() : void {
	...
	if(hasSword){
		itemsContent[0] = GUIContent(itemlistSwordName, a_items[0].icon, "An unbelievably gorgeous sword\nGives " + itemlistSwordStrength + " Attack "
		+ itemlistSwordAgil + " Agility " + itemlistSwordAgil + " Intelligence " + itemlistSwordLuck + " luck");
		hasNothing = false;
		//inventorySize++;
	}
    if(hasStaff){
		itemsContent[0] = GUIContent(itemlistStaffName, a_weapons[2].icon, "A fine Staff\nGives " + itemlistStaffStrength + " Attack "
		+ itemlistStaffAgil + " Agility " + itemlistStaffAgil + " Intelligence " + itemlistStaffLuck + " luck");
		hasNothing = false;
		//inventorySize++;
	}
	if(hasBow){
		itemsContent[0] = GUIContent(itemlistBowName, a_weapons[1].icon, "A lovely Bow\nGives " + itemlistBowStrength + " Attack "
		+ itemlistBowAgil + " Agility " + itemlistBowAgil + " Intelligence " + itemlistBowLuck + " luck");
		hasNothing = false;
		//inventorySize++;
	}
	if(hasArmor){
		itemsContent[1] = GUIContent(itemlistArmorName, a_items[1].icon, "A sturdy armor\nGives " + itemlistArmorStrength + " Attack "
		+ itemlistArmorAgil + " Agility " + itemlistArmorAgil + " Intelligence " + itemlistArmorLuck + " luck");
		hasNothing = false;
		//inventorySize++;
	}
	...
	if(hasHelm){
		itemsContent[2] = GUIContent(itemlistHelmName, a_items[2].icon, "A stylish helm\nGives " + itemlistHelmStrength + " Attack "
		+ itemlistHelmAgil + " Agility " + itemlistHelmAgil + " Intelligence " + itemlistHelmLuck + " luck");
		hasNothing = false;
		//inventorySize++;
	}
	else{
...
//set up equipment tab for inventory
...
//show weapon that is equipped in status window
...
//show armor that is equipped in status window
...
//show accessories that are equipped in stats window
...
//show equipped skill in status window
private function ShowSkill () : void{
	...
	for(var i : int = 0; i<in_items; i++) {
		if(i==0){
		itemsContent[0] = GUIContent(t_skillBox);
		}else{
		itemsContent[1] = GUIContent(a_skills[0]);
		}
	}
